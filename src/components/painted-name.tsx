...
// The hero name. On a first visit it starts blue and a pixel robot walks over the first name with a paint roller,
// painting it white line by line, then celebrates and leaves. The last name stays blue.
export function PaintedName({ firstName, lastName }: { firstName: string; lastName: string }) {
  ...
  return (
    <h1 id="hero-title" ref={titleRef} className="painted-name">
      ...
      <span ref={painterRef} className="painter" aria-hidden="true" style={{ width: 11 * pixel, height: 12 * pixel } as CSSProperties}>
        <RobotSprite pose={pose} mood={mood} />
        {rollerHeight > 0 && (
          <svg className="painter-roller" width={rollerX + 5 * pixel} height={12 * pixel + rollerHeight} style={{ left: 0, top: 0 }}>
            <line x1={10 * pixel} y1={7.5 * pixel} x2={rollerX + 2 * pixel} y2={12 * pixel} stroke="#8f9bb3" strokeWidth={pixel} />
            <rect x={rollerX} y={12 * pixel} width={4 * pixel} height={rollerHeight} rx={pixel} fill="#f1f2f4" />
            <rect x={rollerX + 3 * pixel} y={12 * pixel} width={pixel} height={rollerHeight} fill="#c9d0de" />
          </svg>
        )}
      </span>
    </h1>
