...
// the tape that holds it together, and the note that it works now.
function ZapIcon({ step, readyLabel, iconRef }: { step: ZapStep | 'intact' | 'ready' | 'done'; readyLabel: string; iconRef: Ref<HTMLSpanElement> }) {
  ...
  return (
    <span ref={iconRef} className={`zap-icon zap-${step}`} aria-hidden="true">
      ...
      {step === 'ready' && <span className="zap-ready font-mono">{readyLabel}</span>}
    </span>
  );
}
...
export function ContactList({ rows, copiedLabel, pendingLabel, zapReady }: { rows: readonly Row[]; copiedLabel: string; pendingLabel: string; zapReady: string }) {
  ...
  // a click during it opens WhatsApp and cuts it short; after it, the button just works.
  function openZap(event: MouseEvent<HTMLAnchorElement>) {
    if (zap === 'intact' && zapGagArmed() && zapIcon.current) {
      ...
      playZapGag(zapIcon.current, setZap, controller.signal).then(() => setZap('ready'), () => setZap('done'));
      return;
    }
            ...
            <span className={`contact-action font-mono${done ? ' contact-action-done' : ''}`}>
              {!row.href ? pendingLabel : (
                <>
                  ...
                  {row.icon === 'whatsapp' ? <ZapIcon step={zap} readyLabel={zapReady} iconRef={zapIcon} /> : <RowIcon kind={done ? 'copied' : row.kind} />}
